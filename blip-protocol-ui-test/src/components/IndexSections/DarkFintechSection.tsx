...
function PremiumFintechSection() {
  ...
  // Reset and replay animation every time the section enters the viewport
  useEffect(() => {
    if (!isInView) {
      // User scrolled away — reset to initial state
      setStage("list");
      setAnimDone(false);
      setShowProtocolLock(false);
      return;
    }
    // User scrolled into view — start the animation sequence
    setStage("list");
    setAnimDone(false);
    const timers = [
