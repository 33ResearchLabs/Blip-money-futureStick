    ...
    <div className="relative z-10 p-10 h-full flex flex-col">
      {(title || subtitle) && (
        <div>
          {title && (
            <h3
              className={`text-3xl font-bold tracking-tight mb-2 ${titleColor ? "text-black" : dark ? "text-black dark:text-white" : "text-black"}`}
            >
              {title}
            </h3>
          )}
          {subtitle && (
            <p
              className={`text-base max-w-[280px] leading-relaxed ${titleColor ? "text-black/60" : dark ? "text-black/60 dark:text-gray-400" : "text-gray-600"}`}
            >
              {subtitle}
            </p>
          )}
