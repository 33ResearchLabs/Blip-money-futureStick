...
import { Rocket, Crown, Gift, ShieldCheck } from "lucide-react";
  ...
  return (
    ...
    <div className={`relative w-full   overflow-hidden bg-black ${className}`}>
      ...
      <div className="grid grid-cols-4 gap-3 px-6  py-5 bg-black">
        {[
          { Icon: Rocket, label: "Be the first to try" },
          { Icon: Crown, label: "Exclusive early access" },
          { Icon: Gift, label: "Special launch offers" },
          ...
          >
            <Icon
              size={28}
              strokeWidth={1.6}
              className="text-[#f4c8a8] shrink-0"
            />
            <span className="text-white text-sm leading-tight">{label}</span>
          </motion.div>
        ))}
      </div>
    </div>
