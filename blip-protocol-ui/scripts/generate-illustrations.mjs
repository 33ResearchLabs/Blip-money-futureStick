...
const SCENES = [
  ...
  // Refer-friends hero — premium dark-stage 3D scene (rendered into a dark
  // media-tile inside the card so it works visually in both light & dark mode)
  {
    file: "refer-friends-hero.png",
    prompt: `A premium 3D product render, 1:1 square, set on a near-black studio backdrop (#0a0a0a) lit from above by a soft warm-orange spotlight. Composition: two stylized 3D minimalist person figures (rounded geometric forms — head ball + body cylinder, no faces, no features) standing close together centered in frame; one rendered in glossy warm Claude-orange #cc785c plastic with soft highlights, the other in matte mid-grey. Behind them, a glowing thin warm-orange #cc785c semi-circular arch frames both figures like a halo. In the foreground, slightly off-center, a 3D round coin disc tilted in 3/4 perspective with a simple embossed activity-pulse glyph (a small heartbeat line) on its face — same warm Claude-orange material. A few tiny warm sparkles around the arch. Cinematic studio lighting, soft glossy reflections. Restrained palette: deep black + warm Claude-orange #cc785c + neutral grey only. Premium fintech product photography aesthetic, Apple / Linear / Stripe lineage. No readable text, no logos, no faces, no painterly brushwork.`,
  },
