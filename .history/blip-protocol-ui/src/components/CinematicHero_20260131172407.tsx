    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (phoneLayerRef.current) {
          phoneLayerRef.current.style.transform = `translate3d(${moveX * 15}px, ${moveY * 15}px, 0)`;
        }

        if (textLayerRef.current) {
          textLayerRef.current.style.transform = `translate3d(${moveX * 8}px, ${moveY * 8}px, 0)`;
        }

        if (dashboardLayerRef.current) {
          dashboardLayerRef.current.style.transform = `translate3d(${moveX * 20}px, ${moveY * 20}px, 0)
